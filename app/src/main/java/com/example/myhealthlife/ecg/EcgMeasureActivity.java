...
import com.example.myhealthlife.ecg.view.CardiographView;
import com.example.myhealthlife.ecg.view.CommonDialog;
import com.example.myhealthlife.ecg.view.EcgMeasurDialog;
...
public class EcgMeasureActivity extends Activity {
    ...
    private List<Integer> mEcgMeasureList2 = new ArrayList<>(); // 心电测量数据
    ...
    Handler mHandler = new Handler() {
        ...
        public void handleMessage(@NonNull Message msg) {
            ...
            if (msg.what == 1) {
                ...
                tvStartFinish.setText(getString(R.string.ecg_measure_start));
            } else if (msg.what == 4) {
        ...
        YCBTClient.appRegisterRealDataCallBack(new BleRealDataResponse() {
            ...
            public void onRealDataResponse(int dataType, HashMap hashMap) {
                if (dataType == Constants.DATATYPE.Real_UploadECG) {
                    ...
                    Log.d("appRegisterRealDataCallBack",data.toString());
                    person(data);
                }
            }
        });
        ...
        dialog.setMessage(message) // 是否结束心电测量
                ...
                .setSingle(false).setOnClickBottomListener(new CommonDialog.OnClickBottomListener() {
                    ...
                    public void onConfirmClick() {
                        ecgMeasureStop();
                        dialog.dismiss();
                        Toast.makeText(EcgMeasureActivity.this, getString(R.string.ecg_measure_finish), Toast.LENGTH_SHORT).show();
                    }

                    @Override
                    public void onCancelClick() {
                        dialog.dismiss();
                        Toast.makeText(EcgMeasureActivity.this, getString(R.string.cancelar), Toast.LENGTH_SHORT).show();
                    }
        ...
        public void onClick(View v) {
            ...
            if (id == R.id.tv_start_finish) {
                if (YCBTClient.connectState() == Constants.BLEState.ReadWriteOK) {
                    ...
                    Toast.makeText(EcgMeasureActivity.this, getString(R.string.home_por_favor_conecte), Toast.LENGTH_SHORT).show();
                }
    ...
    private void ecgMeasureStop() {
        YCBTClient.appEcgTestEnd(new BleDataResponse() {
            ...
            public void onDataResponse(int code, float ratio, HashMap resultMap) {
                ...
                Log.d("onDataResponse",mEcgMeasureList.toString());
                Log.d("onDataResponse",mEcgMeasureList2.toString());
                StringBuilder stringBuilder = new StringBuilder();
                for (int i = 0; i < mEcgMeasureList.size(); i++) {
                    stringBuilder.append(mEcgMeasureList.get(i)+"\n");
                }
                LogToFileUtils.write("ecg.txt", stringBuilder.toString());
                if (code == 0) {
...
//         AIDataBean mAiResult = AIData.a().getAIResult();
        AITools.getInstance().getAIDiagnosisResult(new BleAIDiagnosisResponse() {
            ...
            public void onAIDiagnosisResponse(AIDataBean aiDataBean) {
                if (aiDataBean != null) {
                    ...
                    System.out.println("chong------heart==" + heart + "--qrstype==" + mDiagnoseType + "--is_atrial_fibrillation==" + isAfib + "--isHasInaccurateECG==" + YCBTClient.isSupportFunction(Constants.FunctionConstant.ISHASINACCURATEECG));


                    HealthNormBean ecgData = AITools.getInstance().getHealthNorm();
                    String str = new GsonBuilder().create().toJson(ecgData);
                    Log.e("TAG", "onAIDiagnosisResponse ecgData：" + str);
    ...
    private void openEcgMeasure() {
        ...
        YCBTClient.appStartMeasurement(1, 5, new BleDataResponse() {
            @Override
            public void onDataResponse(int code, float ratio, HashMap resultMap) {
                System.out.println("chong------code==" + code);
                if(code == 0){
        ...
        YCBTClient.appEcgTestStart(new BleDataResponse() {
            ...
            public void onRealDataResponse(int i, HashMap hashMap) {
                if (hashMap != null) {
                    System.out.println("chong----------" + hashMap.toString());
                    ...
                    if (i == Constants.DATATYPE.Real_UploadECG) {
                        List<Integer> data = (List<Integer>) hashMap.get("data");
                        List<Integer> originalData = (List<Integer>) hashMap.get("originalData");
                        mEcgMeasureList2.addAll(originalData);
                        Log.d("appRegisterRealDataCallBack",data.toString());
                        person(data);
