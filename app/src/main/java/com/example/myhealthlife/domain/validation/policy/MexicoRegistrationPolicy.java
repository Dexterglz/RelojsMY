...
public class MexicoRegistrationPolicy implements RegistrationPolicy {
    private final NameValidator nameValidator;
    private final EmailValidator emailValidator;
    private final PasswordValidator passwordValidator;
    private final AgeValidator ageValidator;
    private final CurpValidator personalIdValidator;

    public MexicoRegistrationPolicy(
            NameValidator nameValidator,
            EmailValidator emailValidator,
            PasswordValidator passwordValidator,
            AgeValidator ageValidator,
            CurpValidator personalIdValidator
    )
    {
    ...
    public RegistrationResult validate(RegisterUserData data) {
        Log.d("REGISTER_UC", "Mexico");

        if(!nameValidator.requiresLastName(data.getName())){
            Log.d("REGISTER_UC", "apellido paterno invalido");
            return new RegistrationResult.Error(DomainError.INVALID_SECOND_LAST_N);
        }

        if(!nameValidator.requiresSecondLastName(data.getName())){
            Log.d("REGISTER_UC", "apellido materno inválido");
            return new RegistrationResult.Error(DomainError.INVALID_SECOND_LAST_N);
        }
